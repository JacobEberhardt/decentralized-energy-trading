...
function _proportionalDistributionn(deltaProducers, deltaConsumers, hhFrom, hhTo) {
    console.log(hhFrom)
    console.log(hhTo);
...
//     console.log(hhTo);
//     const isMoreAvailableThanDemanded = deltaProducers > deltaConsumers;
//     const factorConsumers = hhTo.map(obj => {
//         return Math.round((Math.abs(obj.meterDelta) / deltaConsumers) * 100000) / 100000 //Javascript being Javascript
//     })

//     const energyReference = isMoreAvailableThanDemanded ? deltaProducers : deltaConsumers;
//     const energyToShare = isMoreAvailableThanDemanded ? deltaConsumers : deltaProducers;

//     for (let i = 0; i < hhFrom.length; i++) {
//         let proportionalFactor = hhFrom[i].meterDelta / energyReference;
//         let shareProducer = Math.round(Math.abs(energyToShare) * proportionalFactor);

//         console.log("proportionalFactor: ", proportionalFactor)
//         for (let j = 0; j < hhTo.length; j++) {
//             let toTransfer = Math.round(shareProducer * factorConsumers[j])
//             console.log("ShareProducer (", shareProducer, ") * factorConsumers (", factorConsumers[j], ") = Amount => ", toTransfer)
//             transfer(i, j, toTransfer);
